...
Important: This version is published on **GitHub Package registry**, before you continue installing, you need to [Authenticate with GitHub using PAT](https://docs.github.com/en/packages/working-with-a-github-packages-registry/working-with-the-npm-registry#authenticating-with-a-personal-access-token)

Run the following command to authenticate with GitHub:

```bash
npm login --scope=@rneui --registry=https://npm.pkg.github.com
```

Then create or modify `.npmrc` in root of your project and add the following line

```bash
@rneui:registry=https://npm.pkg.github.com
```

And install pkgs using yarn or npm
...
:::note Alternatively, you can install from GitHub repo

You don't need PAT for installing via this method

```bash
# @rneui/base
yarn add @rneui/base@rneui/react-native-elements#base

# @rneui/themed
yarn add @rneui/themed@rneui/react-native-elements#themed
```
...
You can install unreleased version, but things may break.
