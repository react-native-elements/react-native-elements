...
export type ListAccordionProps = ListItemProps & {
  isExpanded?: boolean;
  icon?: IconNode;
  expandIcon?: IconNode;
...
const Accordion: RneFunctionComponent<ListAccordionProps> = ({
  children,
  isExpanded,
  icon,
  expandIcon,
  ...
  return (
    <>
      <ListItem {...props}>
        {React.isValidElement(content) ? content : <ListContent />}
        {!noIcon && (
          <Animated.View
            style={{
              transform: [
...
export default withTheme(Accordion, 'ListAccordion');
